package ru.binaryunicorn.coloria.common.utils
...
object OpenGlUtils {
    ...
    fun createShader(shaderType: Int, sourceCode: String): Int {
        ...
        if (shaderId == GLES20.GL_FALSE) {
            Log.e(App.LOGTAG, "Шейдер не создан (на этапе указания типа)")
            return 0
        }
    ...
    fun createShaderProgram(vertexShaderId: Int, fragmentShaderId: Int): Int {
    ...
    fun makeProjectionFrustrimMatrix(
        left: Float, right: Float,
        bottom: Float, top: Float,
        near: Float, far: Float
    ): FloatArray {
